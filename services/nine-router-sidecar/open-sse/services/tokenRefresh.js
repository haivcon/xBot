...
const REFRESH_HANDLERS = {
  "gemini-cli": (c, log) => refreshGoogleToken(c.refreshToken, GOOGLE_OAUTH_CLIENT.clientId, GOOGLE_OAUTH_CLIENT.clientSecret, log),
  antigravity: (c, log) => refreshGoogleToken(c.refreshToken, ANTIGRAVITY_OAUTH_CLIENT.clientId, ANTIGRAVITY_OAUTH_CLIENT.clientSecret, log),
...
async function _getAccessTokenInternal(provider, credentials, log) {
  if (provider === "gemini") {
    return refreshGoogleToken(credentials.refreshToken, GOOGLE_OAUTH_CLIENT.clientId, GOOGLE_OAUTH_CLIENT.clientSecret, log);
  }
